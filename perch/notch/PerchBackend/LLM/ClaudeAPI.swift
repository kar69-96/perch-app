...
class ClaudeAPI {
    ...
    /// System prompt for the vision-gate screen-need classifier. The reply is a
    /// single token, so the instruction is terse and unambiguous.
    private static let visionGateClassifierSystemPrompt = """
    ...
    /// a deterministic deictic guard first, then a tiny YES/NO classify call over
    /// the same `/chat` → OpenRouter path the answers use (untagged, so unmetered).
    /// Any failure (HTTP error, ambiguous reply) returns `true` so the caller falls
    /// back to capturing the screen — reliability over savings.
    func classifyNeedsScreen(
        ...
        // faster and immune to a small model misrouting them to the blind text
        ...
        // Anthropic Messages body (the Worker's /chat forces openrouter/auto +
        // reasoning-exclude, so a reasoning model can't burn this tiny budget on
        // hidden thinking and skip the YES/NO). Untagged → unmetered: the classifier
        // is not a billable message, only the answer it routes to is.
        let body: [String: Any] = [
            "model": model,
            "max_tokens": 5,
            "system": Self.visionGateClassifierSystemPrompt,
            "messages": messages
        ]
        ...
        do {
            ...
            guard let httpResponse = response as? HTTPURLResponse,
                ...
                print("⚠️ Vision-gate classifier non-2xx (status=\(statusCode)) — "
            ...
            if verdict == "NO" {
                print("🚪 Vision gate: NO screen needed → text-only path")
                return false
            }
            ...
            print("⚠️ Vision-gate classifier error: \(error) — defaulting to capture screen")
            return true
        }
    }
